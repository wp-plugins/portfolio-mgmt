 ...
 * @version 1.0.0
 * @since 1.1.6 Fixing error in text domain
...
function wap8_portfolio_help_tabs_content( $tab = 'wap8_portfolio_help_meta' ) {
	...
	if ( $tab == 'wap8_portfolio_help_meta' ) { // if custom meta data tab is active
		...
		ob_start(); // opening an output buffer to store the HTML content ?>
			...
			<p><?php _e( 'The Case Study Information meta box contains optional fields for attaching post meta data to a post. If your currently active theme does not automatically support the display of this data, you will need to customized your templates accordingly.', 'portfolio-mgmt' ); ?></p>

			<h3><?php _e( 'Display Custom Meta Data in your Theme', 'portfolio-mgmt' ); ?></h3>

			<p><?php _e( 'To easily display this content in your theme, you will need to use the <code>get_post_meta()</code> function <strong>inside of the loop</strong>. This function must take three parameters to work properly&mdash; <code>$post_id</code>, <code>$key</code> and <code>$single</code>.', 'portfolio-mgmt' ); ?></p>

			<p><?php _e( 'Here is an example of how to display the client name, wrapped with a <code>&lt;p&gt;</code> tag, within your theme: <code>&lt;?php echo &apos;&lt;p&gt;&apos; . get_post_meta( $post-&gt;ID, &apos;_wap8_client_name&apos;, true ) . &apos;&lt;/p&gt;&apos;; ?&gt;</code>', 'portfolio-mgmt' ); ?></p>

			<h3><?php _e( 'Portfolio Manager Custom Meta Keys', 'portfolio-mgmt' ); ?></h3>

			<p><?php _e( 'Below, you will find the three keys for the Case Study Information meta box.', 'portfolio-mgmt' ); ?></p>
	...
	if ( $tab == 'wap8_portfolio_help_template_tags' ) { // if template tags tab is active
		...
		ob_start(); // opening an output buffer to store the HTML content ?>
			...
			<p><?php _e( 'Included with Portfolio Mgmt. are eight template tags for you to display a list of custom taxonomies associated with a portfolio post.', 'portfolio-mgmt' ); ?></p>

			<h3><?php _e( 'Available Template Tags', 'portfolio-mgmt' ); ?></h3>

			<p><?php _e( 'These template tags <strong>must be placed inside the loop</strong> in order for theme to work as intended. We urge you to wrap your code with the conditional check of <code>&lt;?php if ( function_exists( &apos;wap8_portfolio&apos; ) ) ?&gt;</code>.', 'portfolio-mgmt' ); ?></p>
			...
			<p><?php _e( 'Comma separated template tags will return a list wrapped with a <code>&lt;p&gt;</code> tag. Unordered lists will be wrapped with the <code>&lt;ul&gt;</code> tag.', 'portfolio-mgmt' ); ?></p>

			<p><?php printf( __( '<strong>Note:</strong> When using a template tag with links, the links will point to an archive for that custom taxonomy. If your currently active theme does not contain <a href="%1s">the optional templates</a>, the next available default templates, in <a href="%2s" target="_blank">the WordPress template hierarchy</a>, will be used.', 'portfolio-mgmt' ), esc_url( admin_url( 'admin.php?page=wap8-portfolio-documentation' ) ), esc_url( 'http://codex.wordpress.org/Template_Hierarchy' ) ); ?></p>
...
function wap8_portfolio_help_tabs() {
    ...
    $screen->add_help_tab( array(
		...
		'title'   => __( 'Custom Meta Data', 'portfolio-mgmt' ),
	...
	$screen->add_help_tab( array(
		...
		'title'   => __( 'Template Tags', 'portfolio-mgmt' ),
